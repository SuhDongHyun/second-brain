...
class CollectionSummary:
    """Summarize collection outcomes for one company and business year.
    Per-report counters preserve created, unchanged, missing, and failed totals."""
    ...
    @property
    def failed(self) -> int:
        """Return the number of reports that failed during collection.
        Missing counter entries are treated as zero for successful runs."""
...
async def collect_company_financials(
    *,
    ...
    """Collect, archive, render, and optionally ingest periodic reports.
    Each report is isolated so one OpenDART failure does not stop the remaining work."""
