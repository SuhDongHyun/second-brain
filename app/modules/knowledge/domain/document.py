...
class MarkdownValidationError(ValueError):
    """Represent an unreadable or structurally invalid knowledge document.
    Parsing failures use one stable error type across YAML and Markdown validation."""


class DocumentMetadata(BaseModel):
    """Validate the front-matter contract for a knowledge document.
    Known fields are constrained while source-specific metadata remains allowed."""
    ...
    def require_timezone(cls, value: datetime | None) -> datetime | None:
        """Require offsets on every supplied document timestamp.
        Optional validity bounds remain allowed when no date is known."""
...
class ParsedMarkdown:
    """Carry a validated and normalized Markdown document into ingestion.
    The immutable value binds source, metadata, content, and identity hash."""
...
def normalize_markdown(text: str) -> str:
    """Canonicalize BOMs, line endings, trailing spaces, and final newline.
    Equivalent Markdown inputs therefore produce the same stored content."""
...
def _canonical_metadata(metadata: DocumentMetadata) -> dict[str, Any]:
    """Serialize metadata for deterministic document identity calculation.
    The declared content hash is omitted to avoid hashing a value into itself."""
...
def compute_content_hash(metadata: DocumentMetadata, content: str) -> str:
    """Compute a stable SHA-256 identity from metadata and normalized content.
    Sorted compact JSON ensures ordering does not create false document versions."""
...
def parse_markdown(path: Path) -> ParsedMarkdown:
    """Read and validate a Markdown file with YAML front matter.
    The result contains normalized body text and a freshly computed content hash."""
    try:
